...
class Account extends Sync
{
    ...
    public function getCollectionDataV2($customerCollection)
    {
        ...
        foreach ($customerCollection as $customer) {
            if(is_array($this->mappingField)){
                foreach ($this->mappingField as $field){
                    if($field['zoho_field'] == 'Created Time' || $field['zoho_field'] == 'Modified Time' || $field['zoho_field'] == 'Last Activity Time'){
            ...
            $data[$number]['Account_Number'] = $customer->getData('entity_id');
