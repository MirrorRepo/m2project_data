...
class Invoice extends Sync
{
    ...
    public function getCollectionDataV2($collections)
    {
        ...
        foreach ($collections as $collection) {
            ...
            if ($collection->getOrder()->getData('customer_is_guest') || $collection->getData('customer_id') == null || $collection->getOrder()->getData('customer_firstname')) {
...
//                        $productVal[$countProd]['line_tax'][0]['value'] = floatval($tax);
            ...
            $data[$number]['$line_tax'][0]['percentage'] = floatval($collection->getData('tax_amount') / $collection->getSubtotal() * 100);
...
//            $data[$number]['$line_tax'][0]['percentage'] = floatval($value['tax_amount'] / $value['sub_total'] * 100);
//            $data[$number]['$line_tax'][0]['name'] = 'Vat';
            $number++;
        }
        $params['data'] = $data;
