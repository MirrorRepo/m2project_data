...
class Data extends \Magento\ConfigurableProduct\Helper\Data
{
    ...
    public function getOptions($currentProduct, $allowedProducts)
    {
        ...
        foreach ($allowedProducts as $product) {
            ...
            if ($key && !$this->moduleManager->isEnabled('Amasty_Stockstatus')) {
                $saleable =  $this->isProductSaleble($product, false);
        ...
        $aStockStatus['is_in_stock'] = $this->isProductSaleble($currentProduct, true);
     ...
     * @return bool
     */
    private function isProductSaleble($product, $isConfigurable)
    {
        $stockItem = $this->stockRegistry->getStockItem($product->getId(), 1);
        $isInStock = $stockItem->getIsInStock();
        if (!$isConfigurable) {
            $isInStock = $isInStock && $this->helper->verifyStock($stockItem);
        }
