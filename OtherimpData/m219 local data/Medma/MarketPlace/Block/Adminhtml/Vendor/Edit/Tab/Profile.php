...
class Profile extends \Magento\Backend\Block\Widget\Form\Generic implements \Magento\Backend\Block\Widget\Tab\TabInterface
{
  ...
  if(isset($model['latitude'])){
   ...
   if(isset($model['age_name'])){
    $agen = $model['age_name'];
    
     $agencyNames = $this->vendorHelper->getAgencyname($agen);
         $agename = $agencyNames['firstname']." ".$agencyNames['lastname'];
   ...
   if(isset($model['longitude'])){
    $lati = $model['longitude'];
   }else{
    $lati = "";
   }
        $fieldset->addField(
