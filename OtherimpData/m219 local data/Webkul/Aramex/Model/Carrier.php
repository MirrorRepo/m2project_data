...
class Carrier extends AbstractCarrierOnline implements \Magento\Shipping\Model\Carrier\CarrierInterface
{
    ...
    */
    ...
    {
        if ($this->getConfigData('entity') == "" ||
            $this->getConfigData('user') == "" ||
            $this->getConfigData('password') == "" ||
            $this->getConfigData('account') == "" ||
            $this->getConfigData('pin') == ""
        ) {
            $this->aramexLogger->info(__("Blank Admin configurations"));
            return false;
        }
    ...
    public function getRequestParam()
    {
        ...
        $unit = $this->_scopeConfig->getValue('carriers/webkularamex/unit_of_measure');
        if ($unit == "K") {
            if ($weight > 30) {
                $this->aramexLogger->info("Weight should be less than 30Kg");
                return false;
            }
        } else {
            if ($weight > 66) {
                $this->aramexLogger->info("Weight should be less than 66 lbs");
                return false;
            }
        }
    ...
    protected function _createShipmentRequest()
    {
        ...
        $params['Shipments']['Shipment']['Details'] = [
            ...
            'DescriptionOfGoods' => __("Shipment Package"),
            'GoodsOriginCountry' => $request->getOrigCountryId(),
            'Services' => '',
            'Items' => $request->getItemDesc(),
        ];
    ...
    public function requestToShipment($request)
    {
        ...
        $packageValidFlag = $this->validatePackageDimensions($packages[1]['params']);
        if (!$packageValidFlag) {
            throw new \Magento\Framework\Exception\LocalizedException(__('Please check package dimensions'));
            return false;
        }
    ...
    /*
    * validate dimesnions of package
    * @param array
    * return bool
    */
    public function validatePackageDimensions ($packagesData) {
        $weight = $packagesData['weight'];
        $weightUnit = $packagesData['weight_units'];
        $length = $packagesData['length'];
        $width = $packagesData['width'];
        $height = $packagesData['height'];
        $dimensionUnits = $packagesData['dimension_units'];

        $weightFlag = $this->validatePackageWeight ($weight, $weightUnit);
        $dimensionsFlag = $this->validatePackageDimesnions ($length, $width, $height, $dimensionUnits);
        return $weightFlag && $dimensionsFlag;
    }

    /*
    * validate package weight according to unit
    * @param int weight, string weight unit
    * return bool
    */
    public function validatePackageWeight ($weight, $weightUnit) {
    ...
    /*
    * validate oackage dimensions according to unit
    * @param int length, int width, int height, string unit
    * return bool
    */
    public function validatePackageDimesnions($length, $width, $height, $dimensionUnits) {
        if ($dimensionUnits == "CENTIMETER") {
            if ($length > 274) {
                return false;
            }
            if ($width > 158) {
