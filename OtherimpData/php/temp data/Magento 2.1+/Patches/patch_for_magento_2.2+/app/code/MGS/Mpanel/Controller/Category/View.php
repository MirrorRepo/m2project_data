    ...
    public function execute()
    {
		if($this->getRequest()->isAjax()){
			...
			if ($category) {
				...
				if($this->getRequest()->getParam('catalogajax')==1){
					$this->getRequest()->getQuery()->set('catalogajax', null);
					$requestUri = $this->getRequest()->getRequestUri();
					$requestUri = preg_replace('/(\?|&)catalogajax=1/', '', $requestUri);
					...
					$leftContainer = $this->_view->getLayout()->getBlock('catalog.leftnav')->toHtml();
					$productContainer = $this->_view->getLayout()->getBlock('category.products')->toHtml();
					
					$result = [
						'success' => true, 
						'html' => [
							'products_list' => $productContainer,
							'filters' => $leftContainer
						],
