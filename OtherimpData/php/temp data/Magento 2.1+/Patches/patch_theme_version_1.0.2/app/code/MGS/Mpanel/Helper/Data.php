...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getHeaderColorSetting($storeId) {
        $setting = [
			...
			'nav.navigation #mainMenu .level0:hover > a, nav.navigation #mainMenu .level0 > a.ui-state-focus' => [
			...
			'nav.navigation li.level0 ul.dropdown-menu li a' => [
			...
			'nav.navigation li.level0 ul.dropdown-menu > li:hover a' => [
	...
	/* Get all videos from pub/media/wysiwyg/$type folder */
	public function getPanelUploadVideo($type){
		$path = 'wysiwyg/'.$type.'/';
		$dir = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath($path);
		$result = [];
        if (is_dir($dir)) {
            if ($dh = opendir($dir)) {
                ...
                foreach ($files as $file) {
                    $file_parts = pathinfo($dir . $file);
                    if (isset($file_parts['extension']) && in_array(strtolower($file_parts['extension']), ['ogg', 'mp4'])) {
                        $result[] = $file;
                    }
                }
            }
    ...
    /* Get panel upload folder pub/media/wysiwyg/$type */
    ...
    public function convertContent($layoutContent, $builderContent){
		$beginMain = '<div class="page-wrapper" data-ratioimage="ratio-'. $this->getStoreConfig('mpanel/catalog/picture_ratio') .'">';
		
        
		// if($this->getStoreConfig('mgstheme/general/sticky_menu')){
		...
		if($this->useBuilder() && $this->isHomepage()){
			...
			$builderContent = '<div class="page messages container">
	...
	public function generateCssByStore($storeId){
	   ...
	   $custom_font = $this->getStoreConfig('mgstheme/fonts/custom_fonts_element', $storeId);
		...
		if(($this->getStoreConfig('color/general/theme_color', $storeId) != '') && ($this->getStoreConfig('color/general/theme_color', $storeId) != 'transparent')){
			$themeColorSetting = $this->getThemecolorSetting($storeId);
		...
		if($this->getStoreConfig('color/header/header_custom', $storeId)){
			$headerColorSetting = $this->getHeaderColorSetting($storeId);
		...
		if($this->getStoreConfig('color/main/main_custom', $storeId)){
			$mainColorSetting = $this->getMainColorSetting($storeId);
		...
		if($this->getStoreConfig('color/footer/footer_custom', $storeId)){
			$footerColorSetting = $this->getFooterColorSetting($storeId);
