...
class AbstractBlock extends \Magento\Framework\View\Element\Template
{
    ...
    public function getUrlProduct($subscription)
    {
        $parentId = $subscription->getParentId();
        if (!$parentId) {
            $parentId = $subscription->getEntityId();
        }
        $product = $this->getProductById($parentId);
        $url = $product->getUrlModel()->getUrl($product);
        if ($product->getTypeId() == 'configurable') {
            $hash = $this->getUrlHash($this->getProductById($subscription->getEntityId()));
            $url = $url . $hash;
        }
