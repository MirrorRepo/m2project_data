...
class Data extends \Magento\Framework\App\Helper\AbstractHelper {
    ...
    \Magento\Framework\App\Helper\Context $context, CatalogProductSync $catalogProductSync, ProductApiHelper $productApiHelper
    ) {
    ...
    public function getCatalogAttributes($syncedProduct = null, $product, $cityId) {
        ...
        if ($syncedProduct !== null) {
            ...
            if ($type == "simple") {
                ...
                $pickUpProductOption = $product->getData('catalog_pickup');

                //$vegNonvegProductOption = $product->getData('catalog_veg_nonveg');
        ...
        $result['pick_up_status'] = $pickUpProductOption;

        $result['veg_nonveg_status'] = $vegNonvegProductOption;
    ...
    public function getProductRatings($productId) {
        $avgRatings = 0;
        if (isset($productId) && !is_null($productId) && $productId > 0) {
            ...
            $sql = "SELECT COUNT(`vote_id`) as totalVotes, SUM(`percent`) as totalRatings FROM " . $tableName .
                    " INNER JOIN " . $reviewTable . " ON " . $reviewTable . ".review_id = " . $tableName . ".review_id "
                    . "WHERE " . $tableName . ".`entity_pk_value` = " . $productId . " AND " . $tableName . ".`rating_id` = 3 AND " . $reviewTable . ".status_id = 1";

            $result = $connection->fetchAll($sql);
            if (count($result) >= 0) {
                $ratings = $result[0];
                if ($ratings["totalVotes"] > 0) {
                    $avgRatings = (float) ($ratings["totalRatings"] / $ratings["totalVotes"]);
                    $avgRatings = round(($avgRatings / 20), 1);
                    return number_format((float) $avgRatings, 1);
                }
            }
        }
        return;
