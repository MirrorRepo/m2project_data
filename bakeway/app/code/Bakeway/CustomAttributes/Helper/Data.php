...
class Data extends \Magento\Framework\App\Helper\AbstractHelper {
    ...
    \Magento\Framework\App\Helper\Context $context, MarketplaceProductFactory $marketplaceProduct, MarketplaceProductCollectionFactory $marketplaceProductCollection, ProductRepository $productRepository, MarketplaceSellerCollectionFactory $marketplaceSellerCollection, PartnerWebApiHelper $partnerApiHelper, ContractCollection $contractCollection, MarketplaceHelper $marketplaceHelper, DeliveryRangeCollection $deliveryRangeCollection, GrabintigrationHhelper $grabintigrationHhelper
    ) {
    ...
    \Magento\Quote\Api\Data\CartInterface $quote, $deliveryType, $deliveryTime, $lastQuoteItem = null, $isOrderPlacing = false, $httpRequestMethod = null
    ) {
        if(!is_null($httpRequestMethod))
            $httpRequestMethod = strtolower($httpRequestMethod);

        if ($lastQuoteItem !== null) {
            $this->validateQuoteItem($quote, $lastQuoteItem, $deliveryType, $deliveryTime, true, $isOrderPlacing, $httpRequestMethod);
        }
        /**
        ...
        if ($quote->getItems()) {
            foreach ($quote->getItems() as $quoteItem) {
                $this->validateQuoteItem($quote, $quoteItem, $deliveryType, $deliveryTime, false, $isOrderPlacing, $httpRequestMethod);
            }
        }
    }
    ...
    $quote, $quoteItem, $deliveryType, $deliveryTime, $isLastItem, $isOrderPlacing, $httpRequestMethod = null
    ) {
        ...
        * @adding logs @BKWYADMIN-1119 @08-10-2018
        */
        ...
        if (is_array($operationalArray) && $operationalArray[$dayKey] != 1) {
            $logger->info("===Bakery will be closed on delivery time===".$quoteItem->getData('item_id')."::".$quoteItem->getData('quote_id'));
            if(!is_null($httpRequestMethod) && strtolower($httpRequestMethod) == "post")
                $this->deleteLastItem($quoteItem, $isLastItem);
            $this->grabintigrationHhelper->getBadrequestExpection('Bakery is closed on selected date.','BAKERY_NOT_OPERATIONAL_ON_SELECTED_DATE');
        }

        if (!is_array($operationalArray)) {
            $logger->info("===Operational days not specified===".$quoteItem->getData('item_id')."::".$quoteItem->getData('quote_id'));
            if(!is_null($httpRequestMethod) && strtolower($httpRequestMethod) == "post")
                $this->deleteLastItem($quoteItem, $isLastItem);
            $this->grabintigrationHhelper->getBadrequestExpection('Bakery does not defined any operational days.','BAKERY_OPERATIONAL_DAYS_UNAVAILABLE');
        }
    ...
    public function getOrderTimeInterval() {
        $interval = $this->scopeConfig->getValue('react_site_settings/react_settings_general/order_time_interval', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        return $interval;
    }
