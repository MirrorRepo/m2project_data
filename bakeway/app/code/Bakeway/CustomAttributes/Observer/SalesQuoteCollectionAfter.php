...
class SalesQuoteCollectionAfter implements ObserverInterface {
    ...
    CartExtensionFactory $extensionFactory, CartItemExtensionFactory $itemExtensionFactory, StoreManager $storeManager, AppEmulation $appEmulation, DeliveryShippingHelper $deliveryShippingHelper, ProductImageHelper $productImageHelper, ProductRepository $productRepository, \Bakeway\ProductApi\Helper\Data $productapiHelper) {
    ...
    public function execute(\Magento\Framework\Event\Observer $observer) {
        ...
        foreach ($quote->getAllItems() as $quoteItem) {
            ...
            //code to set value for message on card
            $itemExtAttr->setMessageOnCard($quoteItem->getData('message_on_card'));

            $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
            $product = $objectManager->create('Magento\Catalog\Model\Product')->load($quoteItem->getProductId());
            $vendorProductCode = $product->getData('vendor_product_code');
            $itemExtAttr->setVendorProductCode($vendorProductCode);
             ...
             * @BKWYADMIN-1160
             */
            $catArray = $productObj->getCategoryIds();
            if (!empty($catArray)) {
                $itemExtAttr->setCategoryId($catArray);
            }
            ...
            $rulePrice = $this->productapihelper->getCatalogRulePrice($productObj->getId());
            $ruleTaxPrice = $this->productapihelper->getCatalogRuleTaxPrice($productObj, $rulePrice);
            if ($rulePrice && $ruleTaxPrice) {
                $itemExtAttr->setCatalogDiscountPrice($rulePrice);
                $itemExtAttr->setCatalogDiscountPriceInclTax($ruleTaxPrice);
            } else {
                $itemExtAttr->setCatalogDiscountPrice("0.00");
                $itemExtAttr->setCatalogDiscountPriceInclTax("0.00");
            }
