    ...
    )
    {
        ...
        if ($quote->getId() && $extensionAttr !== null)
        {
            try
            {
                ...
                $logger->info("----------------- Store Name in Database : ".$quoteModel->getStoreUniqueName()." for quote: ".$quote->getId()."-----------------");
                $logger->info("----------------- Store Name Request : ".$extensionAttr->getStoreUniqueName()." for quote: ".$quote->getId()."-----------------");
                /*
                 * BKWYADMIN-1143 Delete Quote items if they belong to different stores
                 */
                if($quoteModel->getIsActive() && ($quoteModel->getStoreUniqueName() != $extensionAttr->getStoreUniqueName()))
                {
                    $itemsCount = count($quoteModel->getItems());
                    $logger->info("----------------- Total Items in cart from Database : ".$itemsCount." for quote: ".$quote->getId()."-----------------");
                    if($quoteModel->getItems() && $itemsCount >= 1)
                    {
                        $logger->info("----------------- Old Store: ".$quoteModel->getStoreUniqueName()." for quote: ".$quote->getId()."-----------------");
                        foreach ($quoteModel->getItems() as $qitem)
                        {
                            $logger->info("Deleting Product: ".$qitem->getProductId());
                            $qitem->delete();
                        }
                        $logger->info("----------------- New Store: ".$extensionAttr->getStoreUniqueName()." for quote: ".$quote->getId()."-----------------");
                    }
                }
                /* BKWYADMIN-1143 Delete Quote items if they belong to different stores Ends */
            ...
            $primaryStoreUniqueName = $quote->getData('primary_store_unique_name');

            if(is_null($primaryStoreUniqueName) || $primaryStoreUniqueName == "")
                $primaryStoreUniqueName = $storeUniqueName;
            ...
            if ($primaryStoreUniqueName !== null) {
                $quote->setData('primary_store_unique_name', $primaryStoreUniqueName);
            }
        ...
        $logger->info("================== Http Method : ". $httpRequestMethod. " ==========================");
        $logger->info("================== Request Path : ". $requestPath. " ==========================");
        $validateItems = true;

        if(strtolower($httpRequestMethod) == "delete")
            $validateItems = false;
        if(strtolower($httpRequestMethod) == "put" && strpos($requestPath, '/items') !== false)
            $validateItems = false;

        $logger->info("================== Validate Items : ". $validateItems. " ==========================");

        if ($lastItem !== null && $validateItems)
        {
            $logger->info("before sending it to validation item : ". $lastItem->getId());
            $this->helper->applyQuoteValidations($quote, $deliveryType, $deliveryTime, $lastItem,false,$httpRequestMethod);
        }
        else
        {
            if($validateItems)
                $this->helper->applyQuoteValidations($quote, $deliveryType, $deliveryTime,null,false,$httpRequestMethod);
        }
