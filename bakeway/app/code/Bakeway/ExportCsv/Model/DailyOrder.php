...
class DailyOrder extends \Magento\Framework\DataObject
{
      /**
 ...
 /**
   ...
   * @return $this
   */
  public function orderexport($prev_day,$orderid = null){ 
   
        /*$fromdate = $this->exportCsvHelper->getStoreWiseFromDate($prev_day);
        $todate   = $this->exportCsvHelper->getStoreWiseToDate($prev_day);*/

        $fromdate = $prev_day ." ".date('00:00:00');
        $fromdate = date("Y-m-d H:i:s", strtotime("-330 minutes", strtotime($fromdate)));

        $todate   = $prev_day . " ".date('23:59:59');
        ...
        $headers = array(
           ...
           if(!is_null($orderid))
           {
              ...
              $orders->addFieldToFilter('created_at', ['lteq' => $todate]);
              $orders->addFieldToFilter('increment_id', ['gt' => $orderid]);
           }
           else
           {
           ...
           $exportfile = $this->getCsvdataFile($headers,$orders,$prev_day);
  
        }

   public function getCsvdataFile($headers,$orders,$prev_day)
      {

        if($prev_day == (date('Y-m-d', strtotime(date('Y-m-01') . ' -1 day'))))
        {
          $start_day = date('Y-m-01', strtotime($prev_day));
          $file = 'reports/orderexport' .$start_day.'to'.$prev_day. '.csv';
          $this->directory->create('reports');
        }
        else
        {
        $file = 'reports/orderexport' .date('Y-m-01').'to'.date('Y-m-t'). '.csv';
        $this->directory->create('reports');
        }

        if(date('Y-m-01') != $prev_day)
          {
    ...
    $orderdetail[ 'Completed At'] = $exportCsvHelper->status($orderdata->getData('label'),$orderdata->getData('updated_at'));
        ...
        $stream->unlock();
         print_r("data saved successfully of date ".$prev_day );
        $stream->close();
        return [
            'type' => 'filename',
            'value' => $file,
