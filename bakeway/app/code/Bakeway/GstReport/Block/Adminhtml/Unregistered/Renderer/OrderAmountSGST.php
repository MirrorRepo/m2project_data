...
class OrderAmountSGST extends \Magento\Backend\Block\Widget\Grid\Column\Renderer\AbstractRenderer
{
    /**
    ...
    public function render(DataObject $row)
    {
        if($row->getName() == "Pune")
        {
            $totalSgst = $row->getTotalAmount() * 0.09;
            $row->setOrderAmountSGST($totalSgst);
        }
        else
        {
            $totalSgst = "NA";
