...
class Email extends \Magento\Framework\App\Helper\AbstractHelper {
    ...
    const XML_PATH_EMAI_STORE_PRODUCT_ENABLE_DISABLE_ALERT_EMAIL = 'vendor_app_settings/bakeway_product_inventory_setting/product_enable_disable_email_template';
    const XML_PATH_REJECT_EMAIL_ADDRESS = 'order/status/rejection_email_list';
    const XML_PATH_PENDING_PAYMENT_EMAIL_TEMPLATE = 'order/status/payment_status_email_template';
    const XML_ORDERS_REPORT_EMAIL_TEMPLATE = 'order/status/order_report_template';
    ...
    public function sendOrderRejectStatusEmailToUser($emailTemplateVariables, $senderInfo, $receiverInfo) {
        ...
        try {
            ...
            //******************** BKWYADMIN-1174 :After order rejection Add email as Bcc  ***********************//
            $rejectEmail = $this->getConfigValue(self::XML_PATH_REJECT_EMAIL_ADDRESS, $this->getStore()->getStoreId());
            if (isset($rejectEmail) && !is_null($rejectEmail)) {
                $rejectEmail = explode(",", $rejectEmail);
                $transport = $this->_transportBuilder->addBcc($rejectEmail);
            }
            //******************************************************************************************************//
            $transport = $this->_transportBuilder->getTransport();
            $transport->sendMessage();
     ...
     * @param Mixed $receiverInfo
     */
    public function sendStoreProductStatusEmail($emailTemplateVariables, $senderInfo, $receiverInfo) {
        $this->_template = $this->getTemplateId(self::XML_PATH_EMAI_STORE_PRODUCT_ENABLE_DISABLE_ALERT_EMAIL);
     ...
     * @param Mixed $receiverInfo
     */
    public function sendPendingPaymentOrders($emailTemplateVariables, $senderInfo, $receiverInfo) {
        $this->_template = $this->getTemplateId(self::XML_PATH_PENDING_PAYMENT_EMAIL_TEMPLATE);
     ...
     */
    public function sendOrdersReport($emailTemplateVariables, $senderInfo, $receiverInfo) {
        $this->_template = $this->getTemplateId(self::XML_ORDERS_REPORT_EMAIL_TEMPLATE);
        $this->_inlineTranslation->suspend();
        $this->generateTemplate($emailTemplateVariables, $senderInfo, $receiverInfo);
        try {
            $transport = $this->_transportBuilder->getTransport();
