...
use Bakeway\Partnerlocations\
Helper\Data as PartnerlocationHelper;
...
use Bakeway\Deliveryrangeprice\Helper\Data as DeliveryrangepriceData;
...
class Email extends \Magento\Framework\Model\AbstractModel {
    ...
    /**
     * @var DeliveryrangepriceData
     */
    protected $deliveryrangepriceData;
     ...
     * @param DeliveryrangepriceData $deliveryrangepriceData
     */
    public function __construct(
    \Magento\Framework\Model\Context $context, \Magento\Framework\Registry $registry, \Webkul\Marketplace\Helper\Data $marketplacehelper, \Magento\Sales\Api\OrderRepositoryInterface $orderRepository, BakewayOrderStatus $bakewayOrderStatus, \Magento\Framework\ObjectManagerInterface $objectManager, AddressRenderer $addressRenderer, OrderstatusEmail $orderstatusEmail, ProductRepositoryInterface $productRepositoryInterface, ReviewRatingHelper $reviewratinghelper, ScopeConfigInterface $scopeconfig, PartnerlocationHelper $partnerlocationHelper, VariableClass $variableClass, Citieshelper $citieshelper, Sellerinstance $sellerinstance, DeliveryrangepriceData $deliveryrangepriceData
    ) {
        ...
        $this->deliveryrangepriceData = $deliveryrangepriceData;
    }
     ...
     * @param string $status
     */
    public function sendStoreProductStatusAlertEmails($storeId, $sellerid, $productsArray, $status) {
        $senderInfo = [];
        $senderInfo = [
            'name' => $this->getAdminusername(),
            'email' => $this->getAdminemail(),
        ...
        $storeName = $this->deliveryrangepriceData->getStoreName($sellerid, $storeId);

        if (!empty($storeName['store_headline']) && !empty($storeName['store_locality_area'])) {
            $storeHeadline = $storeName['store_headline'];
            $storeLocalityArea = $storeName['store_locality_area'];
            $emailTempVariables['store_headline'] = $storeHeadline;
            $emailTempVariables['store_locality_area'] = $storeLocalityArea;
        }

        $productName = [];
        foreach ($productsArray as $product) {
            $productObj = $this->productRepositoryInterface->getById($product);
            $productName[] = $productObj->getName();
        }

        $productsArray = implode(", ", $productName);

        $emailTempVariables['products'] = $productsArray;

        $emailTempVariables['status'] = $status;

        $emailTempVariables['count'] = count($productName);


        $emailAlertStatus = $this->objectManager->get('Bakeway\StoreCatalog\Helper\Data')->getStoreProductsStatus();

        if ($emailAlertStatus == 0) {
            return;
        }

        try {
            $all_EmailAddress = $this->objectManager->get('Bakeway\StoreCatalog\Helper\Data')->getStoreProductsStatusReceiverEmails();
            $all_EmailAddress = explode(",", $all_EmailAddress);
            foreach ($all_EmailAddress as $emailAddress) {
     ...
     */
    public function sendPendingPaymentOrderstoSales($orderData, $receiverEmails) {
        ...
        $emailTempVariables['ordersdata'] = $orderData;
        $emailTempVariables['subject'] = "Please, Find Pending Payment Orders Ids";

        try {
            $this->objectManager->get(
                    'Bakeway\OrderstatusEmail\Helper\Email'
            )->sendPendingPaymentOrders(
                    $emailTempVariables, $senderInfo, $receiverInfo
            );
        } catch (\Exception $e) {
            echo $e->getMessage();
     ...
     * @param array $receiverEmails
     */
    public function sendOrderReports($orderData) {
        $senderInfo = [];
        $receiverInfo = [];
        ...
        $emailTempVariables['ordersdata'] = implode(" ", $orderData);
        $emailTempVariables['subject'] = "Weekly Inventory Update - Bakeway ";

        try {
            $all_EmailAddress = $this->objectManager->get('Bakeway\StoreCatalog\Helper\Data')->getProductsReportStatusReceiverEmails();
            $all_EmailAddress = explode(",", $all_EmailAddress);
            foreach ($all_EmailAddress as $emailAddress) {
