...
class Data extends \Magento\Framework\App\Helper\AbstractHelper {
    ...
    public function getBkwyFirstLevelCategories()
    {
        $_CatArray = [];
        $_catname = "";
        $rootCategoryId = $this->storeManager->getStore(self::CURRENT_STORE_ID)->getRootCategoryId();
        $categoryObj = $this->categoryRepository->get($rootCategoryId);
        $subcategories = $categoryObj->getChildrenCategories();

        foreach($subcategories as $category)
        {
          $_CatArray[] = $category->getId();
          //$_CatArray[] = array("id" => $category->getId(), "label" => $category->getName());
        }
        sort($_CatArray);
        return !empty($_CatArray) ? $_CatArray : "";
    }
