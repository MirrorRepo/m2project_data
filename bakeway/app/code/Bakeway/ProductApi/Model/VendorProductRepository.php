...
class VendorProductRepository implements VendorProductRepositoryInterface {
    ...
    SellerHelper $sellerHelper, \Webkul\Marketplace\Model\SellerFactory $sellerFactory, \Bakeway\ProductApi\Helper\Data $productapiHelper, \Magento\Catalog\Api\ProductRepositoryInterface $productRepository, ProductImageHelper $productImageHelper, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Store\Model\App\Emulation $appEmulation, \Magento\Catalog\Model\ProductFactory $productFactory, JoinProcessorInterface $extensionAttributesJoinProcessor, \Magento\Catalog\Helper\Data $catalogData, VendorProduct $vendorProduct, ProductFilterHelper $productFilterHelper, DeliveryrangeHelper $deliveryrangeHelper, HomedeliveryHelper $homedeliveryHelper, StoreCatalogHelper $storeCatalogHelper, MarketplaceHelper $marketplaceHelper, GrabintigrationHelper $grabintigrationHelper
    ) {
     ...
     */
    public function getProductsdetails($id, $sku, $store = null) {
        ...
        /**
         * store catalouge products check for requested store
         */
        $sellerId = $this->homedeliveryHelper->getSelleridFSku($sku);

        $_productId = $_Model->getId();

        if (empty($sellerId)) {
            throw new LocalizedException(__('Seller not defined for product'));
        }

        $storeCatalougeStatus = $checkProductisDisable = false;

        $allCityCaseFlag = false;

        if (isset($store) && !empty($store)) {

            $allCityNameArray = \Bakeway\Vendorapi\Model\Resource\VendorProductRepository::ALL_CITY_NAME_FOR_FILTER;
            if (in_array($store, $allCityNameArray)) {
                $allCityCaseFlag = true;
            }

            if ($allCityCaseFlag === false) {
                $storeCatalougeStatus = $this->storeCatalogHelper->checkProdAvailibilty($sellerId, $store, $_productId);
                if ($storeCatalougeStatus === true) {
                    $this->grabintigrationHelper->getFourNotFoundExpection('Requested product is disabled in system.', "PRODUCT_IS_DISABLED");
                }
            }

            if ($allCityCaseFlag === true) {

                $checkProductisDisable = $this->storeCatalogHelper->checkAllCityProductCase($sellerId, $_productId);
               
                if ($checkProductisDisable === true) {
                    $this->grabintigrationHelper->getFourNotFoundExpection('Requested product is disabled in system.', "PRODUCT_IS_DISABLED");
                }
            }
        }
        ...
        /**
         * checks for standalone bakeries
         */
        $checkStandAloneBakeryProduct = false;
        $checkSellerType = $this->marketplaceHelper->isConglomerate($sellerId);
        if (empty($checkSellerType)) {
            $checkStandAloneBakeryProduct = $this->storeCatalogHelper->checkProductStatus($sellerId, $_productId);
        ...
        switch ($_ProductArray['type_id']):
            ...
            case "simple":
                ...
                /**
                 * get product attribute pick up value
                 */
                //$productPickUpValue = $_Model->getPickUp();
                //$productPickUpOption = $ExtensionAttObject->setPickupStatus($productPickUpValue);
                ...
                if ($_Model->getId()) {
                    if(!in_array($store, CatalogSeoRepository::ALL_LOC_ARRAY) && !is_null($store) && $store != "")
                        $sizeThreshold = $this->deliveryrangeHelper->getSellerSizeThreshold($id, $store);
                    else
                        $sizeThreshold = $this->deliveryrangeHelper->getSellerSizeThreshold($id);
                    if (!empty($sizeThreshold))
                        $ExtensionAttObject->setMaxAvailDelWeight($sizeThreshold);
                }
            ...
            case "configurable":
                 ...
                 * get product attribute pick up value
                 */
                //$productPickUpValue = $product->getPickUp();
                //$productPickUpOption = $ExtensionAttObject->setPickupStatus($productPickUpValue);
