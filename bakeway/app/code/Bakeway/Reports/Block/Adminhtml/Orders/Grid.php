	...
	*/
	...
	protected function _prepareCollection()
	{
		...
		try {
			...
			$collection->getSelect()->joinLeft(array('mo' => 'marketplace_orders'),
					...
					array('mo.seller_id', 'mo.payment_gateway_fee', 'mo.coupon_amount', 'mo.tcs_amount'));
			...
			$collection->addFilterToMap('name', 'bc.name');
                        $collection->addFilterToMap('tcs_amount',
					'mo.tcs_amount');
	...
	protected function _prepareColumns()
	{
		...
		if ($block) {
  		...
  		if($this->exportCsvAuthorization())
  			{
     ...
     * @return bool
     */
    protected function exportCsvAuthorization()
    {
        return $this->_authorization->isAllowed('Bakeway_Reports::report_csv_file_authorization');
    }

     /**
	 * add custom export data button on order details report grid
	 */      
	 public function getMainButtonsHtml()
	{    
	    $dir = \Magento\Framework\App\Filesystem\DirectoryList::MEDIA."/reports";
	    if (!is_dir($dir))
             mkdir($dir);         
        $data = scandir($dir);
        $output = array_diff($data, [".", ".."]);
        $data = implode("/", $output);
		...
		$addButton = $this->getLayout()->createBlock('Magento\Backend\Block\Widget\Button')
			...
			'label'     => 'Export Data',
    		'id'        => 'export',
			'onclick'   => 'submitFormInPopUp("'.$data.'","'.$dir.'")',
			'class'     => 'task'
		))->toHtml();
		if($data !== "")
