...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    const ORDER_STATUS_COMPLETED_SMS = "Your order has been successfully delivered. We look forward to be part of your celebration. Kindly Review us on ";
     ...
     */
    public function sendNewOrderSmsToCustomer($mobileNumber, $lastOrderId, $sellerId = null, $trackShortLink = null ,$deliveryType = null)
    {
     
        $deliveryMessage = "";
        if(isset($deliveryType) && !empty($deliveryType)){
            if($deliveryType == "home"){
               $deliveryMessage = "It would be delivered";   
            }else{
               $deliveryMessage = "Order can be picked up";     
            }
        }
        
        $message = self::NEW_ORDER_CUSTOMER_SMS.$lastOrderId .', has been placed with the baker. '.$deliveryMessage.' as scheduled '.$trackShortLink;
    ...
    public function sendNewOrderSms($sellerId, $lastOrderId)
    {
        ...
        if ($order->getId()) {
            ...
            if (isset($customerMobile)) {
                ...
                $this->sendNewOrderSmsToCustomer($customerMobile, $orderIncrementId, $sellerId, $trackShortLink ,$deliveryType);
            }
        }
    }
    ...
    public function sendOrderStatusChangeSms($sellerId, $order)
    {
        ...
        switch ($status) {
            ...
            case \Bakeway\Vendorapi\Model\OrderStatus::STATUS_PARTNER_REJECTED :
                ...
                $message = 'Your order with id '.$orderIncrementId.' has not been accepted by the '.$partnerName.', due to '.$reason.'. Auto-refund of the deducted money is initiated. Meanwhile, we would be happy to assist you with a reorder from a different bakery. (For any queries call us: +91-7447766330)';
                $this->sendSms($customerMobile, $message);
                break;
        ...
        if($count > 0) {
            ...
            if($channelEmailNoiification === true){
                foreach ($collection as $item) {
                    ...
                    $supportSubject = "Order for " .$item["store_owner_name"]. " Order Id's" .$item["order_ids"];
                    try{
                        $this->orderStatusEmail->sendNotificationEmail($email,$item["total_order"],$item["order_ids"],$item["store_owner_name"],$content, $subject);
                        $this->orderStatusEmail->sendNotificationEmail($supportEmail,$item["total_order"],$item["order_ids"],$item["store_owner_name"],$content, $supportSubject);
                    }catch(Exception $e){
                        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/pushnoti_accept_reject_order.log');
                        $logger = new \Zend\Log\Logger();
                        $logger->addWriter($writer);
    ...
    public function makeBitlyShortUrl($url, $format = 'xml',$version = '2.0.1')
    {
        ...
        /* Validate the file */
        $file_headers = @get_headers($bitly);
