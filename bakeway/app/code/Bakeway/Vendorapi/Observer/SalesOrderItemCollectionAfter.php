...
class SalesOrderItemCollectionAfter implements ObserverInterface {
    ...
    \Magento\Framework\ObjectManagerInterface $objectManager, ProductRepository $productRepository, ProductImageHelper $productImageHelper, StoreManager $storeManager, AppEmulation $appEmulation, OrderItemExtensionFactory $extensionFactory, \Bakeway\HomeDeliveryshipping\Helper\Data $deliveryshoppinghelper,\Magento\Catalog\Model\ProductFactory $_productFactory
    ) {
    ...
    public function execute(\Magento\Framework\Event\Observer $observer) {
        ...
        foreach ($itemCollection as $item) {
            ...
            //code to set message on card
            $extensionAttributes->setMessageOnCard($item->getMessageOnCard());
            ...
            if (!empty($item->getProductId())) {
                ...
                if ($product->getId())
                {
                    $prodCategory = $this->productRepository->create()->getById($item->getProductId());
                    $catArray = $prodCategory->getCategoryIds();
                    if (!empty($catArray)) {
                        $extensionAttributes->setCategoryId($catArray);
                    }                    
                }
            }
