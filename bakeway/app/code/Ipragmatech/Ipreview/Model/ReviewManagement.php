...
use Ipragmatech\Ipreview\Api\ReviewInterface;
...
class ReviewManagement implements ReviewInterface
{
    ...
    public function getReviewsList($productId)
    {
        ...
        $product = $objectManager->get('Magento\Catalog\Model\Product')
            ->load($productId);
        /*
         * Commented Code as using our custom code to average the rating
         */         
        /* $this->_reviewFactory->create()->getEntitySummary($product,$storeId);//$this->_storeManager->getStore()->getId());
        ...
        {
            $totalMarks = 5 * $count;
            $ratingSummary = (($marks/$totalMarks) * 5);            
        }
        else
           $ratingSummary = 0;
            
        $reviewData[] = [
            "product_ratings"    => round($ratingSummary,1),
            "count"              => $count,
            "reviews"            => $reviewArray
        ];
