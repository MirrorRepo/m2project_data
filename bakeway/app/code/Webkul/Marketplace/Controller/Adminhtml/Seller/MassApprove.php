...
class MassApprove extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        foreach ($collection as $item) {
            ...
            $sellerModel->setIsSeller(1);
            $sellerModel->setUpdatedAt($this->_date->gmtDate());
            $sellerModel->save();
            ...
            if ($sellerProduct->getSize()) {
                ...
                /**
                 * BKWYADMIN-1100 
                 * commenting set Product Data function
                 */
                /*
            ...
            /* email id and name for getting alert of disapprove seller */
            /* start */

            $dissStoremail = $helper->getDisapprosellerEmails();
            ...
            $emailTempVariablesofSalesteam['myvar1'] = $item->getSellerId();
            $emailTempVariablesofSalesteam['myvar2'] = $seller->getName();
            $emailTempVariablesofSalesteam['sellerbusinessName'] = $sellerbusinessName;
            $emailTempVariablesofSalesteam['cityName']=$sellerCity;
            if(!empty($dissStoremail)):
                $dissStoename = $helper->getDisapprosellerName();
                ...
                $all_EmailAddress = $dissStoremail;
                $all_EmailAddress = explode(",",$all_EmailAddress);
                $this->_objectManager->get('Webkul\Marketplace\Helper\Email')->sendApprovedSellerEmail($all_EmailAddress,$emailTempVariablesofSalesteam, $senderInfoCustom);
            endif;
            /* end */
    ...
    protected function _getAclResource()
    {
       ...
       if (in_array($action, ['massapprove'])) {
            $action = 'massapprove';
        }
        switch ($action) {
            case 'index':
            case 'massapprove':
                $aclResource = 'Webkul_Marketplace::massapprove';
                break;
         }
        return $aclResource;
    }
