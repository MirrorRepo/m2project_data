    ...
    public function execute()
    {
        ...
        if ($isPartner == 1) {
            try {
                ...
                $imageSize = getimagesize(\Magento\Framework\App\Filesystem\DirectoryList::MEDIA.'/tmp/catalog/product'.$resultData['file']);
                $width = $imageSize[0];
                $height = $imageSize[1];
                
                if($width >1000 || $height >1000)
                {
                  throw new LocalizedException(__('file size is too big'));
                }
                else{
