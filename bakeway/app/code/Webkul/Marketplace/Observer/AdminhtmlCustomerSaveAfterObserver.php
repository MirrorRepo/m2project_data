...
class AdminhtmlCustomerSaveAfterObserver implements ObserverInterface {
    ...
    public function execute(\Magento\Framework\Event\Observer $observer) {
        ...
        if (!isset($postData['store_city'])) {
            $postData['store_city'] = $avaCollection->getStoreCity();
        }
        ...
        if ($this->isSeller($customerid)) {
            ...
            if (empty($errors)) {
                ...
                if (isset($_FILES['logo_pic'])) {
                    ...
                    {
                    chmod($_FILES['logo_pic']['tmp_name'],0775);
                    $size = getimagesize($_FILES['logo_pic']['tmp_name']);
                    $width = $size[0];
                    $height = $size[1];
                      if($width >254 || $height > 160)
                      {
                        throw new LocalizedException(__('logo pic should be less than or equals 254px * 160px.'));
                        return;
                      }
                    }
                 ...
                 */
                $sellerRating = "";
                if(isset($postData['average_ratings']) && !empty($postData['average_ratings'])){
                    $sellerRating = $postData['average_ratings'];
                }
                //upload cheque image
                ...
                $value->setSellerRating($sellerRating);
                $value->save();
                /* save commision log */
