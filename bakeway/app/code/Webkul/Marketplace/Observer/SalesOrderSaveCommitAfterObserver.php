...
class SalesOrderSaveCommitAfterObserver implements ObserverInterface {
    ...
    \Magento\Framework\Event\Manager $eventManager, \Magento\Framework\ObjectManagerInterface $objectManager, \Magento\Customer\Model\Session $customerSession, \Magento\Framework\Stdlib\DateTime\DateTime $date, PayoutsHelper $payoutsHelper
    ,ScopeConfigInterface $config,  SalesOrder $order ) {
    ...
    public function execute(\Magento\Framework\Event\Observer $observer) {
        ...
        /**
         * code for CRM Auto Sync
         */
        
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/zohocrm_order_log.log');
        ...
        if ($this->getConfigValue($this->pathEnable)) {
              ...
              /** @var \Magento\Sales\Model\Order $order */
                 ...
                 $logger->info("--sync order id ".$increment_id);
                 $this->_order->sync($increment_id);
                 $logger->info("--sync completed for order id ".$increment_id);
        }
        ...
        if ($order->getState() == 'complete') {
            ...
            $ordercollection = $this->_objectManager->create(
                    ...
                    ->addFieldToFilter(
                    'cpprostatus', \Webkul\Marketplace\Model\Saleslist::PAID_STATUS_PENDING
            );
            $taxPaidByBakeway = 0;
